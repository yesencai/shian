...
export class DataModule {
    ...
    onConnectClick() {
        ...
        this.storage.get('RegCode').then((value1) => {
            if (!value1) {
                self.websocket.registered_connectwebsocket(this.device.uuid);
                Emitter.register(this.ttConst.TT_GetConnetAliYunNotification, self.connectClick, self);
            } else {
                // l4uattL0H4i
    ...
    connectClick(name, mCustomData) {
        self.tool.SdkInit(self.OnLogEchoCallback, self.OnEventCallback, self.OnRecvCallback, "1", mCustomData);
        self.tool.SdkStart();
        Emitter.remove(this.ttConst.TT_GetConnetAliYunNotification, self.removeEmitter, self);
    }
    ...
    OnRecvCallback(SrcType, SrcID, PackData) {
        Emitter.fire("TT_ConnectTimeOut");
        console.log('数据回调PackData' + PackData);
        if (PackData === '已没有更多历史记录！') {
        ...
        switch (iPackID) {
            ...
            case self.ttConst.ACK_CLIENT_GET_HISTOR_SECOND_DEVICE: {
                self.onSecondeDeviceProtectionDeviceHistroyListResponse(PackData);
                break;
            }
            ...
            case self.ttConst.ACK_CLIENT_GETINFO: {
                self.onGetPersonInfo(PackData);
                break;
            }
            case self.ttConst.ACK_CLIENT_SETINFO: {
                self.onChangePersonResponse(PackData);
                break;
            }
            case self.ttConst.ACK_CTRL_DEVICE: {
                self.onRelayControlResponse(PackData);
                break;
            }
            case self.ttConst.ACK_GET_DEVICE_DATA: {
                self.onRelayStatuResponse(PackData);
                break;
            }
            case self.ttConst.ACK_CLIENT_CHARGE: {
                self.onWeChatPayResponse(PackData);
                break;
            }
            case self.ttConst.ACK_CLIENT_GET_ALARMHISTOR_FIRST_DEVICE: {
                self.onOneDeviceAlarmDeviceHistroyListResponse(PackData);
                break;
            }
            case self.ttConst.ACK_CLIENT_GET_ALARMHISTOR_SECOND_DEVICE: {
                self.onSecondeDeviceAlarmProtectionDeviceHistroyListResponse(PackData);
                break;
            }
        }
    }
    ...
    //获取个人信息
    onGetPersonInfo(PackData) {
        //余额
        var balance = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GETINFO_BALANCE);
        //昵称
        var nickeName = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GETINFO_NICKNAME);
        //头像
        var headImage = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GETINFO_HEAD);
        //性别
        var sex = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GETINFO_SEX);
        Emitter.fire(this.ttConst.TT_GetPersonInfoNotification, balance, nickeName, headImage, sex);
    }
    //学习状态回调
    onChangePersonResponse(PackData) {
        var mFlag = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_SETINFO_SUCCESS);
        var message = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_SETINFO_ERRORINFO);
        Emitter.fire(this.ttConst.TT_ChangePersonInfoNotification, mFlag, message);
    }
    //控制继电器回调
    onRelayControlResponse(PackData) {
        var switchStatu = this.common.getFieldValue(PackData, this.ttConst.ACK_CTRL_DEVICE_STATUS);
        Emitter.fire(this.ttConst.TT_RelayControlNotification, switchStatu);
    }
    //查询继电器状态
    onRelayStatuResponse(PackData) {
        var statu = this.common.getFieldValue(PackData, this.ttConst.ACK_GET_DEVICE_DATA_STATUS);
        var memory = this.common.getFieldValue(PackData, this.ttConst.ACK_GET_DEVICE_DATA_MEMORY);
        var time = this.common.getFieldValue(PackData, this.ttConst.ACK_GET_DEVICE_DATA_DELAYTIME);
        Emitter.fire(this.ttConst.TT_RelayControlStatusNotification, statu, memory, time);
    }
    onWeChatPayResponse(PackData) {
        var userName = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_CHARGE_USERNAME);
        var orderId = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_CHARGE_ORDERID);
        var price = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_CHARGE_PRICE);
        var appId = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_CHARGE_APPID);
        var mchId = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_CHARGE_MCH_ID);
        var nonce = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_CHARGE_NONCE);
        var sign = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_CHARGE_SIGN);
        var prepayId = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_CHARGE_PREPAY_ID);
        var timeStame = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_CHARGE_TIMESTAMP);
        var url = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_CHARGE_URL);
        Emitter.fire(this.ttConst.TT_WeChatPayNotification, userName, orderId, price, appId, mchId, nonce, sign, prepayId, timeStame, url);
    }
    //二级设备历史记录
    onSecondeDeviceProtectionDeviceHistroyListResponse(PackData) {
        var deviceId = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GET_HISTOR_SECOND_DEVICEID);
        var historyTime = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GET_HISTOR_SECOND_HISTORYTIME);
        var historyData = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GET_HISTOR_SECOND_HISTORYDATA);
        Emitter.fire(this.ttConst.TT_GETSECONDEDEVICEHISTORYLIST_NOTIFICATION_NAME, deviceId, historyTime, historyData);
    }
    //一级设备报警记录
    onOneDeviceAlarmDeviceHistroyListResponse(PackData) {
        var deviceId = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GET_ALARMHISTOR_FIRST_DEVICEID);
        var historyTime = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GET_ALARMHISTOR_FIRST_HISTORYTIME);
        var historyData = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GET_ALARMHISTOR_FIRST_HISTORYDATA);
        Emitter.fire(this.ttConst.TT_GETDEVICEALARMHISTORYLIST_NOTIFICATION_NAME, deviceId, historyTime, historyData);
    }
    //二级设备报警记录
    onSecondeDeviceAlarmProtectionDeviceHistroyListResponse(PackData) {
        var deviceId = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GET_ALARMHISTOR_SECOND_DEVICEID);
        var historyTime = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GET_ALARMHISTOR_SECOND_HISTORYTIME);
        var historyData = this.common.getFieldValue(PackData, this.ttConst.ACK_CLIENT_GET_ALARMHISTOR_SECOND_HISTORYDATA);
        Emitter.fire(this.ttConst.TT_GETSECONDEDEVICEALARMHISTORYLIST_NOTIFICATION_NAME, deviceId, historyTime, historyData);
    }
}