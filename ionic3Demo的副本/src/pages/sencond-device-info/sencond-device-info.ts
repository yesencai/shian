...
import { SecondHistoryPage } from '../second-history/second-history';
...
import { SecondAlarmHistoryRecordPage }from '../second-alarm-history-record/second-alarm-history-record'
...
export class SencondDeviceInfoPage {
  ...
  seleteItem(item) {
    if (item.index == 2) {
      ...
      this.navCtrl.push(SecondHistoryPage, {
        deviceInfo: this.userInfo
      });
    }else if (item.index == 4) {
      this.navCtrl.push(SecondAlarmHistoryRecordPage, {
        deviceInfo: this.userInfo
      });
    }
  }
