...
/**
 * Generated class for the AlarmHistoryRecordPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-alarm-history-record',
  templateUrl: 'alarm-history-record.html',
})
export class AlarmHistoryRecordPage {
  itemData = [];
  listData = [];
  deviceInfo;
  showGreeting = true;
  constructor(public serve: RegistCode, public navCtrl: NavController, public navParams: NavParams, public common: Common, public tomato: Tomato, public ttConst: TTConst, public modalCtrl: ModalController) {
    ...
    let self = this;
    Emitter.register(this.ttConst.TT_GETDEVICEALARMHISTORYLIST_NOTIFICATION_NAME, self.onHistoryDeviceResponse, self);
  }
  ...
  ionViewWillUnload() {
    ...
    let self = this;
    Emitter.remove(this.ttConst.TT_GETDEVICEALARMHISTORYLIST_NOTIFICATION_NAME, self.removeEmitter, self);

  }

  //获取设备布防历史记录列表
  getDeviceHistoryList() {
    var dat = this.common.MakeHeader(this.ttConst.CLIENT_GET_ALARMHISTOR_FIRST_DEVICE) +
      this.common.MakeParam(this.ttConst.CLIENT_GET_ALARMHISTOR_FIRST_DEVICEID, this.deviceInfo.deviceId) +
      this.common.MakeParam(this.ttConst.CLIENT_GET_ALARMHISTOR_FIRST_DEVICENUM, "1");
  ...
  doInfinite(infiniteScroll) {
    setTimeout(() => {
      var dat = this.common.MakeHeader(this.ttConst.CLIENT_GET_ALARMHISTOR_FIRST_DEVICE) +
        this.common.MakeParam(this.ttConst.CLIENT_GET_ALARMHISTOR_FIRST_DEVICEID, this.deviceInfo.deviceId) +
        this.common.MakeParam(this.ttConst.CLIENT_GET_ALARMHISTOR_FIRST_DEVICENUM, (this.itemData.length + 1).toString());
  ...
  showCalendar() {
    ...
    myCalendar.onDidDismiss((date: CalendarResult, type: string) => {
      ...
      var dat = this.common.MakeHeader(this.ttConst.CLIENT_GET_ALARMHISTOR_FIRST_DEVICE) +
        this.common.MakeParam(this.ttConst.CLIENT_GET_ALARMHISTOR_FIRST_DEVICEID, this.deviceInfo.deviceId) +
        this.common.MakeParam(this.ttConst.CLIENT_GET_ALARMHISTOR_FIRST_DEVICETIME, time);
      this.common.SendStrByParent(this.tomato.DSTTYPE_SERVER, this.serve.getServeId(), dat);
