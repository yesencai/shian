...
/**
 * Generated class for the SecondAlarmHistoryRecordPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-second-alarm-history-record',
  templateUrl: 'second-alarm-history-record.html',
})
export class SecondAlarmHistoryRecordPage {
  itemData = [];
  listData = [];
  deviceInfo;
  history;
  ...
  constructor(public serve: RegistCode, public navCtrl: NavController, public navParams: NavParams, public common: Common, public tomato: Tomato, public ttConst: TTConst, public modalCtrl: ModalController) {
    ...
    let self = this;
    Emitter.register(this.ttConst.TT_GETSECONDEDEVICEALARMHISTORYLIST_NOTIFICATION_NAME, self.onHistoryDeviceResponse, self);
  }
  ...
  ionViewWillUnload() {
    ...
    let self = this;
    Emitter.remove(this.ttConst.TT_GETSECONDEDEVICEALARMHISTORYLIST_NOTIFICATION_NAME, self.removeEmitter, self);

  }

  //获取设备布防历史记录列表
  getDeviceHistoryList() {
    var dat = this.common.MakeHeader(this.ttConst.CLIENT_GET_ALARMHISTOR_SECOND_DEVICE) +
