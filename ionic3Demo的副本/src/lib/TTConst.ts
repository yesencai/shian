...
export class TTConst {
  ...
  CLIENT_CHARGE_APRICE = 2; // 标价金额，必填参数，Int，示例值：88。解释：订单总金额，单位为分，详见支付金额
  // 服务器回复
  ACK_CLIENT_CHARGE = 0x2019;
  ACK_CLIENT_CHARGE_USERNAME = 1;
  ACK_CLIENT_CHARGE_ORDERID = 2; // 订单号
  ACK_CLIENT_CHARGE_PRICE = 3; // 金额
  ACK_CLIENT_CHARGE_APPID =4;
  ACK_CLIENT_CHARGE_MCH_ID =5;
  ACK_CLIENT_CHARGE_NONCE =6;
  ACK_CLIENT_CHARGE_SIGN = 7;
  ACK_CLIENT_CHARGE_PREPAY_ID =8;
  ACK_CLIENT_CHARGE_TIMESTAMP =9;
  ACK_CLIENT_CHARGE_URL = 10;
  ...
  // 27.获取账户信息
  CLIENT_GETINFO = 0x101D;
  CLIENT_GETINFO_USERNAME = 1;
  // 服务器返回
  ACK_CLIENT_GETINFO = 0x201D;
  ACK_CLIENT_GETINFO_BALANCE = 1; // 余额
  ACK_CLIENT_GETINFO_NICKNAME = 2; // 昵称
  ACK_CLIENT_GETINFO_HEAD = 3; // 头像
  ACK_CLIENT_GETINFO_SEX = 4; // 性别
  // 30.设置账户信息
  CLIENT_SETINFO = 0x101E;
  CLIENT_SETINFO_USERNAME = 1; // 账号（手机号）
  CLIENT_SETINFO_NICKNAME = 2; // 昵称
  CLIENT_SETINFO_HEAD = 3; // 头像
  CLIENT_SETINFO_SEX = 4; // 性别
  // 服务器返回
  ACK_CLIENT_SETINFO = 0x201E;
  ACK_CLIENT_SETINFO_SUCCESS = 1;
  ACK_CLIENT_SETINFO_ERRORINFO = 2;
  // 31.获取第一级设备告警历史记录
  CLIENT_GET_ALARMHISTOR_FIRST_DEVICE = 0x101F;
  CLIENT_GET_ALARMHISTOR_FIRST_DEVICEID = 1;
  CLIENT_GET_ALARMHISTOR_FIRST_DEVICETIME = 2; // 默认为当天，格式20180509，也可指定，APP自由实现
  CLIENT_GET_ALARMHISTOR_FIRST_DEVICENUM = 3; // 1表示最近10条记录，2表示第11到20条记录，以此类推
  // 服务器返回多条数据，每条数据的内容如下：
  ACK_CLIENT_GET_ALARMHISTOR_FIRST_DEVICE = 0x201F;
  ACK_CLIENT_GET_ALARMHISTOR_FIRST_DEVICEID = 1;
  ACK_CLIENT_GET_ALARMHISTOR_FIRST_HISTORYTIME = 2; // 历史数据发生的时间
  ACK_CLIENT_GET_ALARMHISTOR_FIRST_HISTORYDATA = 3; // 历史数据的内容

  // 32.客户端获取第二级设备告警历史记录，App->服务器
  CLIENT_GET_ALARMHISTOR_SECOND_DEVICE = 0x1020;
  CLIENT_GET_ALARMHISTOR_SECOND_DEVICEID = 1;
  CLIENT_GET_ALARMHISTOR_SECOND_DEVICETIME = 2;
  CLIENT_GET_ALARMHISTOR_SECOND_DEVICENUM = 3; // 1表示最近10条记录，2表示第11到20条记录，以此类推
  // 服务器返回多条数据，每条数据的内容如下：
  ACK_CLIENT_GET_ALARMHISTOR_SECOND_DEVICE = 0x2020;
  ACK_CLIENT_GET_ALARMHISTOR_SECOND_DEVICEID = 1;
  ACK_CLIENT_GET_ALARMHISTOR_SECOND_HISTORYTIME = 2; // 历史数据发生的时间
  ACK_CLIENT_GET_ALARMHISTOR_SECOND_HISTORYDATA = 3; // 历史数据的内容


  /*********************继电器控制********************* */
  //1.继电器控制
  CTRL_DEVICE = 0x3001;
  CTRL_DEVICE_CHANNELID = 1; //子设备编号'1'=1,'2'=2,'0'=all
  CTRL_DEVICE_ACTION = 2;    //动作 '1'= 继电器闭合，‘0’=继电器断开
  CTRL_DEVICE_DELAYTIME = 3; //动作时间，默认0，仅对闭合的动作有效。0:表示永久闭合，直到发指令断开它，时间单位0.1秒 
  CTRL_DEVICE_CUSTOMDATA = 4; //用户回调数据，原样返回，可缺省

  //回复
  ACK_CTRL_DEVICE = 0x3002;
  ACK_CTRL_DEVICE_CHANNELID = 1; //子设备编号'1'=1,'2'=2,'0'=all
  ACK_CTRL_DEVICE_STATUS = 2; //开关状态：通道1状态 + ',' + 通道2状态  1
  ACK_CTRL_DEVICE_MEMORY = 3; //是否断电记忆：通道1记忆状态 + ',' + 通道2记忆状态 
  ACK_CTRL_DEVICE_DELAYTIME = 4; //剩余关闭时间:道1时间+ ',' +通道2时间
  ACK_CTRL_DEVICE_CUSTOMDATA = 5; //用户回调数据，原样返回，可缺省

  //2.设置是否断电记忆状态
  SET_SHUTDOWN_MEMORY = 0x3003;
  SET_SHUTDOWN_MEMORY_CHANNELID = 1; //子设备编号'1'=1,'2'=2,'0'=all
  SET_SHUTDOWN_MEMORY_MEMORY = 2; //'1'=需要断电记忆，‘0’=不需要断电记忆
  SET_SHUTDOWN_MEMORY_CUSTOMDATA = 3; //用户回调数据，原样返回，可缺省

  //回复
  ACK_SET_SHUTDOWN_MEMORY = 0x3004;
  ACK_SET_SHUTDOWN_MEMORY_CHANNELID = 1; //子设备编号'1'=1,'2'=2,'0'=all
  ACK_SET_SHUTDOWN_MEMORY_MEMORY = 2; //当前状态'1'=需要断电记忆，‘0’=不需要断电记忆
  ACK_SET_SHUTDOWN_MEMORY_CUSTOMDATA = 3; //用户回调数据，原样返回，可缺省

  //3.查询
  GET_DEVICE_DATA = 0x3005;
  GET_DEVICE_DATA_CUSTOMDATA = 1;
  //回复
  ACK_GET_DEVICE_DATA = 0x3006;
  ACK_GET_DEVICE_DATA_STATUS = 1; //开关状态：通道1状态 + 通道2状态 各1字节
  ACK_GET_DEVICE_DATA_MEMORY = 2; //是否断电记忆：通道1状态 + 通道2状态 各1字节
  ACK_GET_DEVICE_DATA_DELAYTIME = 3;  //剩余关闭时间
  ACK_GET_DEVICE_DATA_CUSTOMDATA = 4; //用户回调数据，原样返回，可缺省
  ...
  TT_GETDEVICEHISTORYLIST_NOTIFICATION_NAME: any = 'TT_GETDEVICEHISTORYLIST'//一级设备历史记录
  TT_GETSECONDEDEVICEHISTORYLIST_NOTIFICATION_NAME: any = 'TT_GETSECONDEDEVICEHISTORYLIST'//二级设备历史记录
  TT_GETDEVICEALARMHISTORYLIST_NOTIFICATION_NAME: any = 'TT_GETDEVICEALARMHISTORYLIST'//一级设备报警记录
  TT_GETSECONDEDEVICEALARMHISTORYLIST_NOTIFICATION_NAME: any = 'TT_GETSECONDEDEVICEALARMHISTORYLIST'//二级设备报警记录
  ...
  TT_ChangePersonInfoNotification: any = 'TT_ChangePersonInfoNotification'//取消报警
  TT_GetPersonInfoNotification: any = 'TT_GetPersonInfoNotification'//取消报警
  TT_GetConnetAliYunNotification: any = 'TT_GetConnetAliYunNotification'//取消报警
  TT_ScanningDeviceNotification: any = 'TT_ScanningDeviceNotification'//扫码使用
  TT_WeChatPayNotification: any = 'TT_WeChatPayNotification'//扫码使用

  /***********************继电器*************************** */
  TT_RelayControlNotification: any = 'TT_RelayControlNotification'
  TT_RelayControlStatusNotification: any = 'TT_RelayControlStatusNotification'
