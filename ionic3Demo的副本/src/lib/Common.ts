...
export class Common {
    ...
    deviceidStatu;
    deviceidDefense;
    TT_ConnectTimeOutNotification: any = 'TT_ConnectTimeOut'
    show = true;
    list = '设备列表';
    timer;
    //构造函数 依赖注入
    constructor(public platform: Platform,
        ...
        self = this;
        Emitter.register(this.TT_ConnectTimeOutNotification, self.onRespon, self);
    }
    ...
    wifiConfigShowLoading(content: string = ''): void {
        if (!self.loadingIsOpen) {
            self.loadingIsOpen = true;
            self.loading = self.loadingCtrl.create({
                content: content
     ...
     * @param content 显示的内容
     */
    eightShowLoading(content: string = ''): void {
        if (!self.loadingIsOpen) {
            self.loadingIsOpen = true;
            self.loading = self.loadingCtrl.create({
                content: content
            ...
            self.loading.present();
            this.timer = setTimeout(() => { //最长显示10秒
                self.loadingIsOpen && self.loading.dismiss();
                self.loadingIsOpen = false;
                if (this.show) {
                    self.codeMessage('网络连接超时，请重试');
                }
            }, 10000);
        }
    };
